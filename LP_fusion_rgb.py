...
# Fusion of two images (RGB or grayscale)
...
def top_fusion(img_1, img_2, sz_h, sz_w):
    ...
    edge_x = np.zeros([img_1.shape[0], ext_x, img_1.shape[2]], img_1.dtype)
    tem_img_1 = np.hstack((edge_x, img_1, edge_x))
    edge_y = np.zeros([ext_y, tem_img_1.shape[1], img_1.shape[2]], img_1.dtype)
    ...
    for y in range(0, img_1.shape[0]):
        for x in range(0, img_1.shape[1]):
            for cc in range(img_1.shape[2]):
                w_1 = ext_img_1[y:y+2*ext_y+1, x:x+2*ext_x+1, cc]
                w_2 = ext_img_2[y:y+2*ext_y+1, x:x+2*ext_x+1, cc]
...
def pyramid_fusion(pyr_1, pyr_2, gray = False):
    pyr_output = []
